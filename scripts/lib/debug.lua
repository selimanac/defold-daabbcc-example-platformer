...
function debug.update()
	...
	debug.draw_aabb(data.player.position.x, data.player.position.y - (const.PLAYER.SIZE.h / 2), 5, 5, vmath.vector4(0, 0, 0, 1))


	imgui.begin_window("Properties", nil, imgui.WINDOWFLAGS_MENUBAR)

	local changed, checked = imgui.checkbox("Debug view", data.debug)
	if changed then
		data.debug = checked
	end
